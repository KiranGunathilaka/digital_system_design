...
def run_verilog_sim(num_vectors: int):
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "If test_bench_tb.v still uses a fixed #time $finish, it may be ending too early or too late."
        )
...
def run_test(stimulus_a, stimulus_b):
    ...
    if not os.path.exists(RESP_Z_FILE):
        raise RuntimeError("Missing resp_z. Verilog did not generate it (writer never ran).")
    ...
    if len(actual_responses) < len(expected_responses):
        raise RuntimeError(
            ...
            "This is usually because test_bench_tb.v hit $finish too early."
        )
...
def rand32():
    return randint(0, (1 << 32) - 1)  # correct: upper bound inclusive
...
def main():
    ...
    # edge cases (keep small while debugging; scale later)
    tests = [
    ...
    # random (reduced; increase only when stable)
    for _ in range(10):
