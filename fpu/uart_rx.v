...
module uart_rx #(
  ...
  // 2-flop synchronizer for async RX pin
  reg rx_ff1, rx_ff2;
  wire rx_s = rx_ff2;
      ...
      case (state)
        ...
        S_DATA: begin
          if (clk_cnt == CLKS_PER_BIT-1) begin
            clk_cnt <= 0;
            data_buf[bit_idx] <= rx_s;  // LSB first
        ...
        S_STOP: begin
          if (clk_cnt == CLKS_PER_BIT-1) begin
            clk_cnt <= 0;
            // stop bit must be 1, otherwise drop the byte
            if (rx_s == 1'b1) begin
              rx_byte <= data_buf;
              rx_dv   <= 1'b1;
            end
